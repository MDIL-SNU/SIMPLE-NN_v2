...
def train(inputs, logfile, data_loader, model, optimizer=None, criterion=None, scheduler=None, epoch=0, valid=False, err_dict=None,start_time=None, test=False):
    ...
    for i, item in enumerate(data_loader):
        ...
        #loss = _loop_for_loss_type1(inputs, item, model, criterion, progress_dict, struct_weight=(not valid))
...
def _loop_for_loss(inputs, item, model, criterion, progress_dict, struct_weight=False):
    ...
    #loss calculation
    loss += inputs['neural_network']['energy_coeff'] * e_loss
    loss *= inputs['neural_network']['loss_scale']
...
#Traning loop for loss 
def _loop_for_loss_type1(inputs, item, model, criterion, progress_dict, struct_weight=False):
