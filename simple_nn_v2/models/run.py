...
from simple_nn_v2.models.data_handler import StructlistDataset, FilelistDataset, _set_struct_dict, _load_collate, filename_collate
...
def train_NN(inputs, logfile, user_optimizer=None):
    ...
    device = _set_device(logfile)
    model = _initialize_model(inputs, logfile, device)
    optimizer = _initialize_optimizer(inputs, model, logfile)
    criterion = torch.nn.MSELoss(reduction='none').to(device=device)    
    
    # Resume job if possible and load dataset & scale_factor, pca
    scale_factor, pca, train_dataset, valid_dataset, best_loss = _load_data(inputs, logfile, model, optimizer)
...
def _initialize_optimizer(inputs, model, user_optimizer=None):
...
def _load_data(inputs, logfile, model, optimizer):
    scale_factor = None
    pca = None
...
def _init_weight(inputs, model):
    ...
    try:
        ...
        if not init_dic:
            ...
            weight_log = "Xavier Normal weight initialization : gain {0:4.2f}\n".format(gain)
            for lin in model.lin:
                if lin == Linear:
