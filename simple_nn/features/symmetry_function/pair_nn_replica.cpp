...
PairREPLICA::PairREPLICA(LAMMPS *lmp) : Pair(lmp) {
  nelements = 0;
  elements = NULL;
  map = NULL;
  //manybody_flag = 1;
...
void PairREPLICA::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // save symvec for replica
    double *rsymvec = new double[nsym];
    for (tt=0; tt < nsym; tt++) {
        rsymvec[tt] = symvec[tt];
    }
    ...
    // replica
    double *replicaE = new double[npot];
    for (nn = 0; nn < npot ; nn++) {
        Net *rnet = &nets[(nn + 1) * (nelements + 1) + ielem];
        for (tt = 0; tt < nsym; tt++) {
            scale1[tt] = rnet->scale[1][tt];
            symvec[tt] = (rsymvec[tt] - rnet->scale[0][tt])/scale1[tt];
        }
        replicaE[nn] = evalNet(symvec, dsymvec, *rnet);
    }
    ...
    double avg2 = 0;
    for (nn = 0; nn < npot; nn++) {
        avg += replicaE[nn]/npot;
        avg2 += replicaE[nn]*replicaE[nn]/npot;
    }
    q[i] = sqrt(avg2 - avg*avg);
...
void PairREPLICA::coeff(int narg, char **arg)
{
  int i,j,n;

  if (narg != 3 + atom->ntypes + npot)
  ...
  nets = new Net[(npot + 1) * (nelements + 1)]; // extra one is used for reading irrelevant elements.

  // read potential file and initialize potential parameters
  read_file(arg[2], 0);
  // read potential foe replica
  // [pot_ref, pot_1, pot_2, ... , pot_n]
  for (i = 0; i < npot; i++) {
    read_file(arg[2 + 1 + nelements + i], i + 1);
  }
...
void PairREPLICA::read_file(char *fname, int potidx) {
  int i,j,k;
  FILE *fp;
  if (comm->me == 0) {
    //read file
  ...
  while (1) {
    ...
    if (stats == 0) { // initialization
      ...
      nnet = potidx * (nelements + 1) + nelements;
      for (i=0; i<nelements; i++) {
        if (strcmp(t_elem,elements[i]) == 0) {
          nnet = potidx * (nelements + 1) + i;
          break;
        }
      }
      if (nnet == nelements) {
...
double PairREPLICA::init_one(int i, int j)
{
...
void PairREPLICA::write_restart(FILE *fp) {}

/* ----------------------------------------------------------------------
   proc 0 reads from restart file, bcasts
------------------------------------------------------------------------- */

void PairREPLICA::read_restart(FILE *fp) {}

/* ----------------------------------------------------------------------
   proc 0 writes to restart file
------------------------------------------------------------------------- */

void PairREPLICA::write_restart_settings(FILE *fp) {}

/* ----------------------------------------------------------------------
   proc 0 reads from restart file, bcasts
------------------------------------------------------------------------- */

void PairREPLICA::read_restart_settings(FILE *fp) {}

/* ---------------------------------------------------------------------- */

double PairREPLICA::single(int i, int j, int itype, int jtype, double rsq,
...
void PairREPLICA::free_net(Net &net) {
...
double PairREPLICA::evalNet(const double* inpv, double *outv, Net &net){
